#!/bin/bash
#
# Purpose: Receive and process SPROUT SSTV captures.
#
# Input parameters:
#   1. Name of SPROUT satellite (i.e. "SPROUT")
...
#
# Example:
#   ./receive_sprout.sh "SPROUT" SPROUT20210208-194829 ./orbit.tle 1612831709 919 31 Southbound E
...
export GAIN=$SPROUT_SSTV_GAIN
export SUN_MIN_ELEV=$SPROUT_SSTV_SUN_MIN_ELEV
export SDR_DEVICE_ID=$SPROUT_SSTV_SDR_DEVICE_ID
if [ "$SPROUT_SSTV_ENABLE_BIAS_TEE" != "usb" ]; then
    export BIAS_TEE=$SPROUT_SSTV_ENABLE_BIAS_TEE
else
    export BIAS_TEE=""
fi
export FREQ_OFFSET=$SPROUT_SSTV_FREQ_OFFSET
export SAT_MIN_ELEV=$SPROUT_SSTV_SAT_MIN_ELEV
...
log "Receive SPROUT SSTV Processes starting...." "INFO"
if [ "$SPROUT_SSTV_ENABLE_BIAS_TEE" == "usb" ]; then
...
if [ "$NOAA_RECEIVER" == "rtl_fm" ]; then
  ...
  ${AUDIO_PROC_DIR}/sprout_record_rtl_fm.sh "${SAT_NAME}" $CAPTURE_TIME "${AUDIO_FILE_BASE}.wav" >> $NOAA_LOG 2>&1
fi
if [ "$NOAA_RECEIVER" == "gnuradio" ]; then
  log "Starting gnuradio record" "INFO"
  ${AUDIO_PROC_DIR}/sprout_record_gnuradio.sh "${SAT_NAME}" $CAPTURE_TIME "${AUDIO_FILE_BASE}.wav" >> $NOAA_LOG 2>&1
fi
...
${IMAGE_PROC_DIR}/sprout_sstv.sh "${AUDIO_FILE_BASE}.wav" "${IMAGE_OUTPUT}/${FILENAME_BASE}.png" >> $NOAA_LOG 2>&1
...
if [ "${ENABLE_SLACK_PUSH}" == "true" ]; then
  slack_push_annotation="SPROUT Slow Scan TV files"
  pass_id=$($SQLITE3 $DB_FILE "SELECT id FROM decoded_passes ORDER BY id DESC LIMIT 1;")
